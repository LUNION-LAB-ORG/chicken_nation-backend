...
/**
 * Fait entrer un inscrit dans la population des prospects et l'en fait sortir
...
export class ConversionListener {
  ...
  constructor(private readonly sync: ConversionSyncService) {}
  ...
  private async synchroniser(customerId: string | undefined, commandeId: string | undefined, origine: string) {
    ...
    try {
      ...
      this.logger.warn(`Suivi prospect (${origine}) du client ${customerId} échoué : ${(e as Error).message}`);
    }
  }
}
