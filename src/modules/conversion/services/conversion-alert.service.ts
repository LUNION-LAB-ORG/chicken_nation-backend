...
import { CampaignStatus, ConversionEventType, NotificationType } from '@prisma/client';
...
import { ConversionNotificationsTemplate } from '../templates/conversion-notifications.template';
...
/**
 * Alertes du module Prospects (cahier §8), dans la cloche du backoffice.
 ...
 */
@Injectable()
export class ConversionAlertService {
  private readonly logger = new Logger(ConversionAlertService.name);
  ...
  /**
   * Prospects affectés dans une campagne en cours, toujours pas appelés après
  ...
  async alerterNonContactes(): Promise<number> {
    ...
    const lignes = await this.prisma.$queryRaw<
      { member_id: string; prospect_id: string; agent_id: string; campaign_id: string; campagne: string; lead_agent_id: string }[]
    >`
      SELECT m.id AS member_id, m.prospect_id, m.agent_id, m.campaign_id, c.name AS campagne, c.lead_agent_id
      FROM "ConversionCampaignMember" m
      JOIN "ConversionCampaign" c ON c.id = m.campaign_id AND c.status = 'ACTIVE'
      JOIN "ConversionProspect" p ON p.id = m.prospect_id AND p.status <> 'CONVERTI' AND p.entity_status <> 'DELETED'
    ...
    await this.events.journaliser(
      lignes.map((l) => ({
        ...
        type: ConversionEventType.ALERTE,
  ...
  async notifierFinCampagne(campagneId: string, indicateurs: { conversions: number; cibles: number }) {
    const c = await this.prisma.conversionCampaign.findUnique({
      where: { id: campagneId },
    ...
    await this.envoyer(
      ConversionNotificationsTemplate.CAMPAGNE_TERMINEE,
      destinataires,
      { campagne: c.name, conversions: indicateurs.conversions, cibles: indicateurs.cibles },
      { kind: 'conversion', campaign_id: campagneId },
    );
  }
  ...
  private async envoyer<T>(
    ...
    try {
      ...
      this.logger.warn(`Notification du module Prospects non envoyée : ${(e as Error).message}`);
    }
  }
}
