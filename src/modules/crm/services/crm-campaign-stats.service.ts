 ...
 * bougent plus quand ses contacts vivent leur vie après elle.
 *
 * Définitions :
  ...
  async resumes(ids: string[]) {
    ...
    const [membres, appels, coupons] = await Promise.all([
      this.prisma.$queryRaw<{ campaign_id: string; cibles: number; conversions: number }[]>`
        ...
        FROM "CrmCampaignMember" WHERE campaign_id IN (${liste}) GROUP BY campaign_id`,
      this.prisma.$queryRaw<{ campaign_id: string; traites: number }[]>`
        SELECT campaign_id, count(DISTINCT contact_id)::int AS traites
        FROM "CrmCall" WHERE campaign_id IN (${liste}) GROUP BY campaign_id`,
      this.prisma.$queryRaw<{ campaign_id: string; coupons: number }[]>`
        SELECT campaign_id, count(*)::int AS coupons
        FROM "CrmCoupon" WHERE campaign_id IN (${liste}) GROUP BY campaign_id`,
    ]);
      ...
      await Promise.all([
        ...
        this.prisma.$queryRaw<{ appels: number; traites: number; joints: number }[]>`
          SELECT count(*)::int AS appels, count(DISTINCT contact_id)::int AS traites,
                 count(DISTINCT contact_id) FILTER (WHERE reached)::int AS joints
          FROM "CrmCall" WHERE campaign_id = ${id}::uuid`,
        this.prisma.$queryRaw<{ envoyes: number; utilises: number; ca: number }[]>`
          ...
          FROM "CrmCoupon" WHERE campaign_id = ${id}::uuid`,
        this.prisma.$queryRaw<{ restants: number }[]>`
          SELECT count(*)::int AS restants
          FROM "CrmCampaignMember" m JOIN "CrmContact" p ON p.id = m.contact_id
          WHERE m.campaign_id = ${id}::uuid AND m.released_at IS NULL AND p.status <> 'CONVERTI'
            AND NOT EXISTS (SELECT 1 FROM "CrmCall" k WHERE k.campaign_id = m.campaign_id AND k.contact_id = m.contact_id)`,
        ...
        this.prisma.$queryRaw<{ jour: string; nombre: number }[]>`
          ...
          FROM (SELECT min(created_at) AS premier FROM "CrmCall" WHERE campaign_id = ${id}::uuid GROUP BY contact_id) t
          GROUP BY 1 ORDER BY 1`,
        this.prisma.$queryRaw<{ jour: string; nombre: number }[]>`
          SELECT to_char(created_at, 'YYYY-MM-DD') AS jour, count(*)::int AS nombre
          FROM "CrmCall" WHERE campaign_id = ${id}::uuid GROUP BY 1 ORDER BY 1`,
        this.prisma.$queryRaw<{ jour: string; nombre: number }[]>`
          SELECT to_char(converted_at, 'YYYY-MM-DD') AS jour, count(*)::int AS nombre
          FROM "CrmCampaignMember" WHERE campaign_id = ${id}::uuid AND converted_at IS NOT NULL GROUP BY 1 ORDER BY 1`,
  ...
  private async parAgent(id: string, equipe: string[]) {
    ...
    >`
      WITH acteurs AS (
        ...
        UNION SELECT agent_id FROM "CrmCall" WHERE campaign_id = ${id}::uuid AND agent_id IS NOT NULL
        UNION SELECT agent_id FROM "CrmCampaignMember" WHERE campaign_id = ${id}::uuid AND agent_id IS NOT NULL
      )
      SELECT u.id, u.fullname,
        (SELECT count(*) FROM "CrmCampaignMember" m WHERE m.campaign_id = ${id}::uuid AND m.agent_id = u.id)::int AS assignes,
        (SELECT count(*) FROM "CrmCall" k WHERE k.campaign_id = ${id}::uuid AND k.agent_id = u.id)::int AS appels,
        (SELECT count(DISTINCT k.contact_id) FROM "CrmCall" k WHERE k.campaign_id = ${id}::uuid AND k.agent_id = u.id)::int AS traites,
