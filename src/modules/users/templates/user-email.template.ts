...
export class UserEmailTemplates {
    ...
    NEW_USER: EmailTemplate<{ actor: User, user: Omit<User, 'password' | 'id'> }> = {
        subject: (ctx) => `Nouvel utilisateur`,
        content: (ctx) => {

            const emailContent = [
                this.emailComponentsService.Title('Nouvel utilisateur'),
                this.emailComponentsService.Message(`${ctx.data.user.fullname} a rejoint votre équipe en tant que ${ctx.data.user.role}`),
    ...
    NEW_MEMBER: EmailTemplate<{ actor: User, user: User & { restaurant: Restaurant }}>  = {
    ...
    WELCOME_USER: EmailTemplate<{ actor: User, user: Omit<User, 'password' | 'id'> }> = {
